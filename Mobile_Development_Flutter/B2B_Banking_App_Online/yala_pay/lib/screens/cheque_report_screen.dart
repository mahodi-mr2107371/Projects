...
class _ChequeReportScreenState extends ConsumerState<ChequeReportScreen> {
  ...
  Future<void> fetchFilteredCheques() async {
    filteredCheques = await ref
        .watch(chequesProvider.notifier)
        .getChequesByDateAndStatusFirestore(fromDate, toDate, selectedStatus);
    setState(() {});
  }

  Future<void> resetFilteredCheques() async {
    filteredCheques = await ref
        .read(chequesProvider.notifier)
        .getChequesByDateAndStatusFirestore(fromDate = DateTime(2024, 1, 1),
            toDate = DateTime.now(), selectedStatus = 'All');
    init = true;
    setState(() {});
  }
